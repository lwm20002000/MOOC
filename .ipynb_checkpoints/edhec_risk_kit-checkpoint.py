...
def get_ffme_returns():
    ...
    me_m = pd.read_csv("data/Portfolios_Formed_on_ME_monthly_EW.csv",
                      header=0, index_col=0, na_values=-99.9)
...
def var_historic(r, level=5):
    ...
    fall below that number and the (100-level) percent are above the number
    """
...
def annualize_rets(r, periods_per_year):
    ...
    We should inder the periods per year
...
def minimize_vol(target_return, er, cov):
    ...
    # setting bounds for scipy.optimize.minimize
    bounds = ((0.0, 1.0),)*n
    return_is_target = {
        'type':'eq',
        'args':(er, ),
        'fun': lambda weights, er: target_return - portfolio_return(weights,er) # write a function to chekc whether the constraint is met or not. Use a lambda (anonymous function)
    }
    weights_sum_to_1 = {
        'type': 'eq',
...
def optimal_weights(n_points, er, cov):
    ...
    """
    # A given min or max return of a given portfolio is the min/max of putting 100% of assets into either the lower or higher returning asset
...
def plot_ef(n_points, er, cov, show_cml = False, style=".-",riskfree_rate=0, show_ew=False, show_gmv=False):
    ...
    """
    weights = optimal_weights(n_points, er, cov) # generating weights for n number of assets
    rets = [portfolio_return(w, er) for w in weights]
    vol = [portfolio_vol(w, cov) for w in weights]
    ef = pd.DataFrame({
        "Returns":rets,
    ...
    if show_cml:
        ...
        r_msr = portfolio_return(w_msr, er) # y
        vol_msr = portfolio_vol(w_msr, cov) # x
        # Add CML
        cml_x = [0, vol_msr]
...
def run_cppi(risky_r, safe_r=None, m=3, start=1000, floor=0.8, riskfree_rate=0.03, drawdown=None):
    ...
    # run this over time. At each stage, track values
    ...
    for step in range(n_steps):
        ...
        risky_w = m*cushion
        # we want to limit allocation to 100% - otherwise leverage or debt
...
def gbm(n_years=10, n_scenarios=1000, mu=0.07, sigma=0.15, steps_per_year=12, s_0=100):
    ...
    Simulating price models using the Brownian Motion Model
    """
    dt = 1/steps_per_year
